...
import { generateLocationSummary, AIResult } from "@/services/aiService";
...
const ControlPanel: React.FC<ControlPanelProps> = ({
  ...
  const [aiSource, setAiSource] = useState<"ai" | "fallback" | "">("");
  ...
  const generateAISummary = useCallback(async () => {
    ...
    try {
      const nearbyFacilities =
        facilities?.slice(0, 5).map((f) => `${f.name} (${f.distance}m)`) || [];

      const result = await generateLocationSummary(
        {
          address: selectedLocation.address,
          facilityCounts: livabilityData.facilityCounts,
          scores: {
            ...
            <div className="flex items-center gap-3 pb-2 border-b border-[hsl(var(--control-border))]">
              ...
              <div>
                <h3 className="text-sm font-semibold text-[hsl(var(--control-primary))]">
                  ...
                  {userMode === "residents"
                    ? "Residents"
                    : userMode === "business-owner"
                    ? "Business"
                    : "Urban Planning"}
                </h3>
                <p className="text-xs text-[hsl(var(--control-primary))]/70">
                  {userMode === "residents"
                    ? t("smart.insights.residents")
                    : userMode === "business-owner"
                    ? t("business.opportunity.analysis")
              ...
              {isGeneratingSummary ? (
                ...
                <div className="space-y-2">
                  <div className="flex items-center gap-2">
                    {aiSource === "ai" ? (
                      <Badge className="bg-emerald-600/80 text-white">AI</Badge>
                    ) : (
                      <Badge
                        variant="outline"
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-3 pb-2 border-b border-[hsl(var(--control-border))]">
            ...
            <div>
              <h3 className="text-sm font-semibold text-[hsl(var(--control-primary))]">
                {userMode === "residents"
                  ? t("current.location.residents")
                  : userMode === "business-owner"
                  ? t("current.location.business")
          ...
          {selectedLocation ? (
            ...
            <div className="p-4 bg-[hsl(var(--control-bg-light))] rounded-xl border border-[hsl(var(--control-border))] text-center control-panel-box">
              ...
              <p className="text-sm text-[hsl(var(--control-primary))]">
                {userMode === "residents"
                  ? t("no.location.selected.residents")
                  : userMode === "business-owner"
                  ? t("no.location.selected.business")
